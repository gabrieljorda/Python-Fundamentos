...
tb_clientes = pd.read_excel("dados/clientes.xlsx")
print(tb_clientes)
...
tb_clientes = pd.read_excel("dados/clientes.xlsx",usecols = ["Nome", "Idade"])
print(tb_clientes)
...
with pd.ExcelWriter("dados/novos_clientes.xlsx") as nova_planilha:
    tb_clientes_aba1.to_excel(nova_planilha, sheet_name="Aba 1", index=False)
    tb_clientes_aba2.to_excel(nova_planilha, sheet_name="Aba 2", index=False)  
